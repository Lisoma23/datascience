...
with st.form("prediction_form"):
    ...
    with col2:
        ...
        segment = st.selectbox("Segment Client", ["Basic", "Medium", "Premium"])
        contract = st.selectbox("Type de Contrat", ["Mensuel", "Annuel (1 an)", "Annuel (2 ans)"])
        channel = st.selectbox("Canal d'acquisition", ["SEO", "Publicité", "Email", "Referral"])
        payment = st.selectbox("Méthode de paiement", ["Carte Bancaire", "PayPal", "Virement"])
    ...
    with col3:
        ...
        session_time = st.number_input("Temps moyen de session (min)", value=15)
    ...
    with col5:
        ...
        monthly_fee = st.number_input("Frais mensuels ($)", value=49)
    ...
    with col6:
        ...
        csat = st.slider("Score CSAT (1-10)", 1, 10, 8)
        ...
        complaint = st.selectbox("Type de réclamation", ["Aucune", "Technique", "Facturation", "Autre"])
        survey = st.selectbox("Dernier sondage", ["Positive", "Neutral", "Negative"])
...
if submit_button:
    ...
    data_to_send = {
        ...
        "marketing_click_rate": click_rate,
        "nps_score": nps,
